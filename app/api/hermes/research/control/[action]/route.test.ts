...
describe("Hermes Research Control API Route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("pauses research generation", async () => {
  ...
  it("resumes research generation", async () => {
    mockGetChatAuthenticatedUser.mockResolvedValue({ _id: "user-1" } as any);
  ...
  it("updates research section content", async () => {
    mockGetChatAuthenticatedUser.mockResolvedValue({ _id: "user-1" } as any);
  ...
  it("exports research report", async () => {
    mockGetChatAuthenticatedUser.mockResolvedValue({ _id: "user-1" } as any);

    const response = await POST(
      new NextRequest("http://localhost", {
        method: "POST",
        body: JSON.stringify({ runId: "r-1", workspaceId: "w-1", requestId: "req-1", format: "markdown" }),
      }),

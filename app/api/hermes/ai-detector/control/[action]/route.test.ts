...
describe("Hermes AI Detector Control API Route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("exports AI detector report", async () => {
    mockGetChatAuthenticatedUser.mockResolvedValue({ _id: "user-1" } as any);

    const response = await POST(
      new NextRequest("http://localhost", {
        method: "POST",
        body: JSON.stringify({ runId: "r-1", workspaceId: "w-1", requestId: "req-1", format: "pdf" }),
      }),

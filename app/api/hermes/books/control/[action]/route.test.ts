...
describe("Hermes Books Control API Route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("pauses book generation", async () => {
  ...
  it("updates book chapter", async () => {
    mockGetChatAuthenticatedUser.mockResolvedValue({ _id: "user-1" } as any);

    const response = await POST(
      new NextRequest("http://localhost", {
        method: "POST",
        body: JSON.stringify({ runId: "r-1", workspaceId: "w-1", requestId: "req-1", chapterIndex: 1 }),
      }),
  ...
  it("exports book manuscript", async () => {
    mockGetChatAuthenticatedUser.mockResolvedValue({ _id: "user-1" } as any);

    const response = await POST(
      new NextRequest("http://localhost", {
        method: "POST",
        body: JSON.stringify({ runId: "r-1", workspaceId: "w-1", requestId: "req-1", format: "epub" }),
      }),
